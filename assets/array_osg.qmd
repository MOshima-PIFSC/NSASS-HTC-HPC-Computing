...
2. Line 15: Change `User.Name` to your user name and `linux.sif` to the name of your container file. 
...
Once the jobs have completed, you can retrieve the results for further analysis on your local computer. The easiest way to do this is to compress all of the directories in `array_lm/inputs`. In the first terminal, logged into OSG, run:  
```{bash}
...
Then on your local compter create a new directory `outputs` to put all of the downloaded results. In the second terminal window, navigate to the outputs folder and run: 
```{bash}
