...
Replace `User.Name` with your NOAA HPC account name, and `bastion` with either `boulder` or `princeton` depending on which 'bastion' you wish to log-in through.
...
## Transferring files to/from Hera via scp using data transfer node (DTN)
...
File transfer takes place via *scp*:
```{bash}
#| echo: true
#| output: false
scp /path/to/local/file User.Name@dtn-hera.fairmont.rdhpcs.noaa.gov:/scratch1/NMFS/project_name/User.Name/
```
replace `User.Name` and `project_name` with your HPC account name and project.
...
```
This will copy your file from your local machine into your home directory on Hera. Simply append additionally directory structure to `/home/User.Name/` to copy it into a sub-directory that you have created on Hera (e.g., `/home/User.Name/sub/dir/`). To transfer files into your *scratch* directory, specify the proper destination path (e.g., `/scratch1/NMFS/project_name/User.Name/` where `project_name` is your HPC account project). 
