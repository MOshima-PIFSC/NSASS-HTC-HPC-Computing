...
* `osg.sub`: This is the HTCondor job submission script that species job requirements and input/output files.
...
The `r nrow(comptime_dt)` jobs run on OSG completed `r round(sum(comptime_dt$OSG_runtime)/60,digits=2)` hours of calculations (`r round(mean(comptime_dt$OSG_runtime),digits=2)` minutes per job) in an elapsed time of `r round(as.numeric(abs(difftime(min(comptime_dt$OSG_start),max(comptime_dt$OSG_end),units="hours"))),digits=2)` hour (@fig-OSG-elapsed).
