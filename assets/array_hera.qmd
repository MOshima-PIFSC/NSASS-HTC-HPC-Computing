...
```{r}
shell(paste0("powershell cd ", file.path(proj_dir, "examples", "hera", "array_lm"), ";tar -czf upload.array_lm.tar.gz array_lm "))
```
...
```{bash}
dos2unix array_lm/inputs/run_lm_hera_array.r array_lm/inputs/hera_job_directories.txt array_lm/slurm_scripts/submit_array_lm.sh
```
6. Submit job  
...
```{bash}
sbatch array_lm/slurm_scripts/submit_array_lm.sh
```
