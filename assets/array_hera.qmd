...
By compressing the entire array_lm folder, you will be able to transfer all of the input data (`inputs/*`) and the submission script (`slurm_scripts/submit_array_lm.sh`) at the same time.  
...
Back in *Terminal A* untar the uploaded directory and move the submission script to the `submit_scripts` directory you made in step 2.
```{bash}
# untar files
tar -xzf upload.array_lm.tar.gz
# move submission scripts 
mv array_lm/slurm_scripts/submit_array_lm.sh submit_scripts/submit_array_lm.sh
```
...
```{bash}
dos2unix array_lm/inputs/run_lm_hera_array.r array_lm/inputs/hera_job_directories.txt submit_scripts/submit_array_lm.sh
```
6. Submit job  
